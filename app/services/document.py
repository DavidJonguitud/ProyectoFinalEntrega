...
class DocumentService:
    ...
    async def get_project_documents(
        self, project_id: int, user_email: str
    ) -> list[Document]:
        access = self.access_repo.get_project_role_by_project_id_and_email(
            project_id, user_email
        )
        if not access:
            raise ValueError(
                "The project does not exist or you do not have permission to access."
            )
    ...
    async def upload_document_for_project(
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported file type '{file_ext}'. Only PDF and DOCX are allowed."
            )

        file_path = await self.storage_service.upload_file(
            file=file, folder=f"project_{project_id}"
        )
    ...
    async def get_document_path_for_download(
        ...
        access = self.access_repo.get_project_role_by_project_id_and_email(
            document.project_id, current_user.email
        )
        if not access:
            raise PermissionError(
                "You do not have permission to access the project of this document"
            )
    ...
    async def update_project_for_authorized_user(
        self, document_id: int, current_user: User, file: UploadFile
    ) -> Document:
        document = self.document_repo.get_document_by_id(document_id)
        if not document:
            raise ValueError("The document does not exist.")
        ...
        new_file_path = await self.storage_service.upload_file(
            file=file, folder=f"project_{document.project_id}"
        )
        ...
        try:
            ...
            logger.exception(f"Could not delete old file {old_file_path}")
    ...
    async def delete_project_for_authorized_user(
        self, document_id: int, current_user: User
    ):
        document = self.document_repo.get_document_by_id(document_id)
        if not document:
