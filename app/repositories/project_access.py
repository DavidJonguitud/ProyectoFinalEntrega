...
class ProjectAccessRepository:
    ...
    def create_project_access(
        self, project_access_data: ProjectAccessCreate
    ) -> ProjectAccess:
    ...
    def get_projects_by_user_email(self, user_email: str) -> list[ProjectAccess]:
        return (
            self.db.query(ProjectAccess).filter(ProjectAccess.email == user_email).all()
        )

    def get_project_role_by_project_id_and_email(
        self, project_id: int, email: str
    ) -> ProjectRole:
        access = (
            self.db.query(ProjectAccess)
            .filter(
                ProjectAccess.email == email, ProjectAccess.project_id == project_id
            )
            .first()
        )
