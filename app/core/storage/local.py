    ...
    async def delete_file(self, file_path: str)-> None:
        try:
            ...
            if os.path.exists(absolute_path):
                ...
                logger.warning(f"Fle to delete does not exist in the disk: {absolute_path}")
        except Exception as e:
            logger.error(f"Error deleting file {file_path}: {e}", exc_info=True)
            raise IOError(f"Error deleting the file from disk: {str(e)}")

    async def get_file_for_download(self, file_path: str) -> str:
        absolute_path = os.path.abspath(file_path)
        if not os.path.exists(absolute_path):
            raise FileNotFoundError(f"The physiical file was not found on the server path: {absolute_path}")
